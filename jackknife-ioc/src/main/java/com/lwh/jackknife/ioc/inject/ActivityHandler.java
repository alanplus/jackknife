...
public class ActivityHandler implements InjectHandler {
    ...
    public String generateLayoutName(SupportV v) {
        ...
        if (!layoutName.endsWith(suffix)) {
            throw new IllegalXmlMappingException("Class name is not ends with \'Activity\'.");
        } else {
            ...
            if (View.class.isAssignableFrom(fieldType)) {
                ...
                try {
                    ...
                    Method findViewByIdMethod = activityClass.getMethod(METHOD_FIND_VIEW_BY_ID,
                            int.class);
                    Object view = findViewByIdMethod.invoke(v, id);
                    if (view != null) {
                        field.set(v, view);
                    }
                } catch (Exception e) {
