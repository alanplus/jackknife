...
//        exit_program(1);
        kill_program();
        LOGI("exit_program(1) from assert_avoptions");
    }
}
...
//    exit_program(1);
    kill_program();
    LOGI("exit_program(1) from abort_codec_experimental");
}
...
//                exit_program(1);
                kill_program();
                LOGI("exit_program(1) from write_packet");
            }
...
//            exit_program(1);
            kill_program();
        LOGI("exit_program(1) from output_packet");
    }
}
...
//    exit_program(1);
    kill_program();
    LOGI("exit_program(1) from do_audio_out");
}
...
//            exit_program(1);
            kill_program();
        LOGI("exit_program(1) from do_subtitle_out");
        return;
    }
...
//    exit_program(1);
    kill_program();
    LOGI("exit_program(1) from do_video_out");
}
...
//            exit_program(1);
            kill_program();
            LOGI("exit_program(1) from do_video_stats");
        }
    }
...
//                    exit_program(1);
                    kill_program();
                    LOGI("exit_program(1) from flush_encoders");
                }
                if (ost->logfile && enc->stats_out) {
                    fprintf(ost->logfile, "%s", enc->stats_out);
                }
...
//            exit_program(1);
            kill_program();
            LOGI("exit_program(1) from do_streamcopy");
        }
        if (ret) {
...
static void check_decode_result(InputStream *ist, int *got_output, int ret)
{
    ...
    if (ret < 0 && exit_on_error){
        //        exit_program(1);
        kill_program();
        LOGI("exit_program(1) from check_decode_result");
    }
...
//            exit_program(1);
            kill_program();
            LOGI("exit_program(1) from decode_audio");
        }
...
//                exit_program(1);
                kill_program();
                LOGI("exit_program(1) from decode_video");
            }
        }
    }
    ...
    while (ist->decoding_needed) {
        ...
        if (ret < 0) {
            ...
            if (exit_on_error){
                //                exit_program(1);
                kill_program();
                LOGI("exit_program(1) from process_input_packet");
            }
...
static void print_sdp(void)
{
    ...
    if (!avc){
        //        exit_program(1);
        kill_program();
        LOGI("exit_program(1) from print_sdp");
    }
...
//            exit_program(1);
            kill_program();
            LOGI("exit_program(1) from init_output_stream_streamcopy");
        }
...
//            exit_program(1);
            kill_program();
            LOGI("exit_program(1) from init_output_stream");
        }
        /*
...
//        exit_program(1);
        kill_program();
        LOGI("exit_program(1) from parse_forced_key_frames");
    }
...
static void set_encoder_id(OutputFile *of, OutputStream *ost)
{
    ...
    if (!encoder_string){
        //        exit_program(1);
        kill_program();
        LOGI("exit_program(1) from set_encoder_id");
    }
    ...
    for (i = 0; i < nb_output_streams; i++) {
        ...
        if (!ost->stream_copy) {
            ...
            if (qsv_transcode_init(ost)){
                //exit_program(1);
                kill_program();
                LOGI("exit_program(1) from transcode_init");
            }
    ...
    if (ret < 0) {
        if (ret != AVERROR_EOF) {
            ...
            if (exit_on_error){
                //                exit_program(1);
                kill_program();
                LOGI("exit_program(1) from process_input");
            }
    ...
    for (i = 0; i < nb_output_files; i++) {
        ...
        if ((ret = av_write_trailer(os)) < 0) {
            ...
            if (exit_on_error){
                //                exit_program(1);
                kill_program();
                LOGI("exit_program(1) from transcode");
            }
...
//Talkboy reset static variables for consecutive use
void kill_program(){
